...
public class Busca_Em_Largura {
	...
	public Busca_Em_Largura(State state, State goal_state) {
		this.state = state;
	...
	public LinkedList<Action> resolve() throws Exception{
		...
		Node current_node = new Node(this.state, null, null, 0, 0);
		
		if(current_node.isGoal(this.goal_state)){
		...
		while(true){
			...
			for (Node children : node.getChildren()) {								
				if(!contains(explored, children.getState()) && !contains(queue, children)){
